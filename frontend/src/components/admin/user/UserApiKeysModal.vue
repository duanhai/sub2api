  ...
  <BaseDialog :show="show" :title="t('admin.users.userApiKeys')" width="wide" @close="handleClose">
    <div v-if="user" class="space-y-4">
      ...
      <div v-else ref="scrollContainerRef" class="max-h-96 space-y-3 overflow-y-auto" @scroll="closeGroupSelector">
        <div v-for="key in apiKeys" :key="key.id" class="grid items-start gap-x-6 gap-y-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800 sm:grid-cols-[minmax(0,1fr)_17rem]">
          ...
          <div class="flex flex-wrap gap-4 text-xs text-gray-500 sm:col-start-1">
          ...
          <form class="sm:col-start-2 sm:row-start-1 sm:row-span-2" @submit.prevent="saveConcurrencyLimit(key)">
            <label :for="`key-concurrency-${key.id}`" class="input-label">{{ t('keys.concurrencyLimit') }}</label>
            <div class="flex items-center gap-2">
              <input
                :id="`key-concurrency-${key.id}`"
                v-model.number="concurrencyLimits[key.id]"
                type="number"
                min="0"
                max="2147483647"
                step="1"
                class="input w-28"
                :aria-describedby="`key-concurrency-hint-${key.id}`"
                :disabled="updatingKeyIds.has(key.id)"
              />
              <button
                type="submit"
                class="btn btn-primary btn-sm"
                :disabled="updatingKeyIds.has(key.id) || Number(concurrencyLimits[key.id]) === (key.concurrency_limit ?? 0)"
              >{{ t(updatingKeyIds.has(key.id) ? 'common.saving' : 'common.save') }}</button>
            </div>
            <p :id="`key-concurrency-hint-${key.id}`" class="input-hint">{{ t('keys.concurrencyLimitHint') }}</p>
          </form>
        </div>
      </div>
    </div>
...
const concurrencyLimits = ref<Record<number, number | string>>({})
...
const load = async () => {
  ...
  try {
    ...
    if (version === requestVersion) {
      ...
      concurrencyLimits.value = Object.fromEntries(apiKeys.value.map(key => [key.id, key.concurrency_limit ?? 0]))
    }
  } catch (error) {
...
const saveConcurrencyLimit = async (key: ApiKey) => {
  if (updatingKeyIds.value.has(key.id)) return
  const limit = Number(concurrencyLimits.value[key.id])
  ...
  try {
    const result = await adminAPI.apiKeys.updateApiKeyConcurrencyLimit(key.id, limit)
    const index = apiKeys.value.findIndex(item => item.id === key.id)
    if (index !== -1) apiKeys.value[index] = result.api_key
    concurrencyLimits.value[key.id] = result.api_key.concurrency_limit ?? 0
    appStore.showSuccess(t('keys.concurrencyLimitSaved'))
  } catch (error: unknown) {
    appStore.showError(error instanceof Error ? error.message : t('keys.failedToSave'))
  } finally {
