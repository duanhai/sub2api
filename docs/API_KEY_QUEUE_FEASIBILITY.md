...
推荐在 **系统设置 → 网关服务 → API Key 并发排队** 中开启功能，设置每个 Key
的等待容量和超时，点击“保存排队设置”。所有正数并发上限的 Key 自动使用自己的
执行上限，无需逐个填写 ID，无需修改 YAML，无数据库迁移。

例如后台给某个 Key 设置并发上限 2，面板设置最多等待 6，则这个 Key 可容纳
2 个执行名额和 6 个等待者。等待容量是每个 Key 独立计算，不是所有 Key 共用。

保存后当前实例的新 HTTP 请求立即使用新设置；其他实例最长约 5 秒同步。
已有排队请求及 WebSocket 连接保留原策略，WebSocket 重连后应用新设置。
共享数据库不可用时保留最近已知策略；尚未成功加载过策略时退回即时准入，
仍强制执行 Key 并发上限，不绕过限制。

兼容旧版：只有从未保存面板设置时，才继续读取原 YAML 规则。首次保存后，
面板对所有 Key 生效，关闭开关也不会重新启用 YAML 规则。
旧配置示例（不再推荐新增）：
...
- 默认关闭；Key 并发上限为 0 时仍不限制并发。
- 面板配置无需重启。仅旧版 YAML 兼容配置仍需各实例一致并在修改后重启。
...
WebSocket 按连接建立时的排队策略启用单一持续 reader，在业务准入阻塞时继续处理
